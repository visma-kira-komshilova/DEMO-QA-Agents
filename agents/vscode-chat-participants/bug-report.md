...
**No initial prompt.** Begin analysis immediately when the user provides error details, per CLAUDE.md execution protocol.
...
If JIRA MCP is available, search before generating:
```
JQL: summary ~ "<error keywords>" AND status != Done AND project = HB
```

- Duplicate found → report existing ticket
- Related (not duplicate) → reference in report
- JIRA MCP unavailable → add note: `**Duplicate check:** Not performed — JIRA MCP unavailable. Verify manually before submitting.`
...
**Space management:** If approaching 900 words, compress Section 8 (Test Data) to bullet points and abbreviate Section 9 manual scenarios to 1 row. Always retain the full 4-row E2E table. Sections 1-7 must not be abbreviated.

---

**Generated reports location:** `reports/bug-report/`
